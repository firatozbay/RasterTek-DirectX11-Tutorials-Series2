...
bool ApplicationClass::Initialize(HINSTANCE hinstance, HWND hwnd, int screenWidth, int screenHeight)
{
	...
	result = m_Model->Initialize(m_D3D->GetDevice(), m_D3D->GetDeviceContext(), "../Engine/data/sphere.txt", L"../Engine/data/blue.dds");
	if(!result)
	{
	...
	result = m_Bitmap->Initialize(m_D3D->GetDevice(), m_D3D->GetDeviceContext(), screenWidth, screenHeight, L"../Engine/data/mouse.dds", 32, 32);
	if(!result)
	{
...
void ApplicationClass::TestIntersection(int mouseX, int mouseY)
{
	...
	XMVECTOR tmp_direction, tmp_origin, tmp_rayOrigin, tmp_rayDirection;
	...
	tmp_origin = XMLoadFloat3(&origin);
	tmp_direction = XMLoadFloat3(&direction);

	// Now transform the ray origin and the ray direction from view space to world space.
	tmp_rayOrigin = XMVector3TransformCoord(tmp_origin, inverseWorldMatrix);
	tmp_rayDirection = XMVector3TransformNormal(tmp_direction, inverseWorldMatrix);

	// Normalize the ray direction.
	tmp_rayDirection = XMVector3Normalize(tmp_rayDirection);
	XMStoreFloat3(&rayOrigin, tmp_rayOrigin);
	XMStoreFloat3(&rayDirection, tmp_rayDirection);
