...
bool BumpModelClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* modelFilename, WCHAR* textureFilename1, WCHAR* textureFilename2)
{
	bool result;
...
bool BumpModelClass::LoadTextures(ID3D11Device* device, ID3D11DeviceContext* deviceContext, WCHAR* filename1, WCHAR* filename2)
{
	bool result;
	...
	result = m_ColorTexture->Initialize(device, deviceContext, filename1);
	if (!result)
	{
		return false;
	}
	...
	result = m_NormalMapTexture->Initialize(device, deviceContext, filename2);
	if (!result)
	{
		return false;
	}
